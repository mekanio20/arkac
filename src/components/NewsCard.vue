<template>
    <router-link :to="`/news/detail/${id}`" class="flex flex-col justify-between space-y-4 sm:space-y-6">
        <div class="relative flex items-center justify-center group cursor-pointer overflow-hidden rounded-[10px]">
            <img :src="image" class="w-full h-[200px] sm:h-[250px] md:h-[285px] object-cover rounded-[10px] transition-all duration-300 group-hover:scale-110 group-hover:brightness-90" />
            <!-- Arrow icon -->
        ...
        <h3 class="font-inter font-semibold text-base sm:text-lg">
            {{ title }}
        </h3>
        <div class="flex items-center font-inter font-medium text-sm sm:text-base">
            <span class="text-arkac-blue-200">{{ category }}</span>
